...
//   const [mesAFacturar, setMesAFacturar] = useState("actual");
...
//     // ✅ FIX Bug 4: Guard anti-doble facturación
//     // Recargar datos frescos de Firebase antes de ejecutar
//     const clientsRef = collection(db, "clients");
//     const clientsSnapFresh = await getDocs(clientsRef);
//     const clientesYaFacturados = [];

//     clientsSnapFresh.docs.forEach((docSnap) => {
//       const clientFresh = { id: docSnap.id, ...docSnap.data() };
//       const estaSeleccionado = clientesSeleccionados.some(
//         (c) => c.id === clientFresh.id,
//       );
//       if (
//         estaSeleccionado &&
//         clienteTieneMesPagadoAdelantado(clientFresh, mesParaFacturar)
//       ) {
//         clientesYaFacturados.push(clientFresh);
//       }
//     });

//     if (clientesYaFacturados.length > 0) {
//       const nombres = clientesYaFacturados
//         .map((c) => `${c.name} ${c.lastName}`)
//         .join(", ");
//       Swal.fire({
//         icon: "warning",
//         title: "Posible doble facturación detectada",
//         html: `
//           <p><strong>${clientesYaFacturados.length}</strong> cliente(s) ya tienen facturado este mes o uno posterior:</p>
//           <p style="color: #f57c00; font-size: 0.9rem;">${nombres}</p>
//           <p>Cerrá este diálogo y regenerá el preview para ver el estado actualizado.</p>
//         `,
//       });
//       return;
//     }
...
//       html: `
//       <p>Se facturarán <strong>${clientesSeleccionados.length}</strong> clientes</p>
//       <p>Total nuevas cuotas: <strong>${totalSeleccionado.toLocaleString("es-AR")}</strong></p>
//       <p>Mes a facturar: <strong>${formatearMesLegible(mesParaFacturar)}</strong></p>
//       ${
//         mesAFacturar === "actual"
...
//           // ✅ FIX Bug 2: No pisar ultimoMesFacturado si el existente es posterior
//           // Esto protege a clientes que pagaron adelantado
//           const ultimoMesExistente = client.ultimoMesFacturado || "";
//           const nuevoUltimoMes =
//             ultimoMesExistente > mesParaFacturar
//               ? ultimoMesExistente
//               : mesParaFacturar;
...
//             estado: nuevoEstado,
//             ultimoMesFacturado: nuevoUltimoMes, // ✅ conserva el más reciente
...
//         <p>Total facturado: <strong>${totalSeleccionado.toLocaleString("es-AR")}</strong></p>
...
//                     value="actual"
//                     control={<Radio />}
//                     label={`Mes Actual (${formatearMesLegible(getMesActual())})`}
//                   />
//                   <FormControlLabel
//                     value="siguiente"
//                     control={<Radio />}
//                     label={`Mes Siguiente (${formatearMesLegible(getMesSiguiente(getMesActual()))})`}
//                   />
//                 </RadioGroup>
//               </Box>
...
//                       <Chip
//                         label={`${getClientesSeleccionados().length}/${previewData.clientesAFacturar.length}`}
//                         color="primary"
//                         size="small"
//                       />
//                     </Box>
...
//                               {client.saldoFavor > 0 && (
//                                 <Chip
//                                   label={`Saldo: ${client.saldoFavor.toLocaleString("es-AR")}`}
//                                   size="small"
//                                   color="success"
...
/**
 * Devuelve true si el cliente ya tiene `mesAVerificar` cubierto
 * (ultimoMesFacturado >= mesAVerificar)
 */
const clienteYaTieneMesCubierto = (client, mesAVerificar) => {
  if (!client.ultimoMesFacturado) return false;
  // Comparación lexicográfica de "YYYY-MM" funciona correctamente
  return client.ultimoMesFacturado >= mesAVerificar;
};

const calcularCuota = (client, activitiesList) => {
  const actividad = activitiesList.find((a) => a.label === client.actividad);
  if (!actividad) return 0;
  return Math.round((actividad.valor * (client.proporcion || 1)) / 100) * 100;
};
...
const MonthlyBillingManager = ({ activities, setIsChange }) => {
  ...
  // ⚠️ mesAFacturar como estado — el valor se PASA EXPLÍCITAMENTE a generarPreview
  const [mesAFacturar, setMesAFacturar] = useState("actual");
  ...
  // Recibe `mesMode` como parámetro para evitar el stale closure.
  const generarPreview = useCallback(
    async (mesMode = "actual", activitiesList = null) => {
      setIsProcessing(true);
      try {
        const actList =
          activitiesList ??
        ...
        const mesBase = getMesActual();
        const mesParaFacturar =
          mesMode === "siguiente" ? getMesSiguiente(mesBase) : mesBase;
        ...
        clientsSnap.docs.forEach((docSnap) => {
          ...
          // Inactivos — excluir
          if (client.estado === "Inactivo") {
            clientesExcluidos.push({ ...client, razon: "Cliente inactivo" });
            return;
          }

          // Sin actividad — excluir
          if (!client.actividad) {
            clientesExcluidos.push({
              ...client,
              razon: "Sin actividad asignada",
          ...
          // ✅ El cliente ya tiene ese mes (o uno posterior) cubierto — excluir
          if (clienteYaTieneMesCubierto(client, mesParaFacturar)) {
            clientesExcluidos.push({
              ...client,
              razon: `Ya tiene facturado hasta ${formatearMesLegible(
                client.ultimoMesFacturado,
              )}`,
            });
            return;
          ...
          const cuota = calcularCuota(client, actList);
          // Deuda anterior visible solo al facturar el mes actual
          const deudaAnteriorTotal =
            mesMode === "actual"
  ...
  const handleOpen = async () => {
    ...
    activities.forEach((a) => {
      activityPrices[a.id] = a.valor;
    });
    setEditingActivityPrices(activityPrices);
    setUpdatedActivities([...activities]);
    await generarPreview("actual", activities);
  };
  ...
  const handleMesChange = (nuevoModo) => {
    setMesAFacturar(nuevoModo);
    // ✅ Pasamos el valor nuevo directamente, no leemos el estado
    generarPreview(
      nuevoModo,
      updatedActivities.length ? updatedActivities : activities,
    );
  };
  ...
  const handleActivityPriceChange = (activityId, newPrice) => {
    const price = parseInt(newPrice) || 0;
    setEditingActivityPrices((prev) => ({ ...prev, [activityId]: price }));
    setUpdatedActivities((prev) =>
      prev.map((act) =>
        act.id === activityId ? { ...act, valor: price } : act,
      ),
  ...
  const handleApplyActivityPrices = async () => {
    ...
    try {
      const batch = writeBatch(db);
      updatedActivities.forEach((activity) => {
        batch.update(doc(db, "activities", activity.id), {
          valor: activity.valor,
        });
      });
      await batch.commit();
      await generarPreview(mesAFacturar, updatedActivities);
      setShowPriceUpdate(false);
      Swal.fire({
        icon: "success",
        title: "Precios actualizados",
      ...
      console.error(error);
      Swal.fire({ icon: "error", title: "Error al actualizar precios" });
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const toggleSelectAll = () => {
    if (!previewData) return;
    const allSelected = previewData.clientesAFacturar.every(
      (c) => selectedClients[c.id],
    );
    const newSel = {};
    previewData.clientesAFacturar.forEach((c) => {
  ...
  const handleEditPrice = (clientId) =>
    setEditingPrices((prev) => ({ ...prev, [clientId]: true }));

  const handleSavePrice = (clientId) =>
    setEditingPrices((prev) => ({ ...prev, [clientId]: false }));

  const handleCancelEdit = (clientId) => {
    const client = previewData.clientesAFacturar.find((c) => c.id === clientId);
    const actList = updatedActivities.length ? updatedActivities : activities;
    setCustomPrices((prev) => ({
      ...prev,
      [clientId]: calcularCuota(client, actList),
    }));
    setEditingPrices((prev) => ({ ...prev, [clientId]: false }));
  };
  ...
  const getClientesSeleccionados = () =>
    previewData
      ? previewData.clientesAFacturar.filter((c) => selectedClients[c.id])
      : [];

  const getTotalSeleccionados = () =>
    getClientesSeleccionados().reduce(
      (sum, c) => sum + (customPrices[c.id] ?? c.cuotaAFacturar),
      0,
    );

  const filteredClientes = previewData
    ? previewData.clientesAFacturar.filter((c) =>
        `${c.name} ${c.lastName}`
          .toLowerCase()
          .includes(searchTerm.toLowerCase()),
      )
  ...
  const ejecutarFacturacion = async () => {
    ...
    if (clientesSeleccionados.length === 0) {
      Swal.fire({
        icon: "warning",
        title: "Sin clientes seleccionados",
        text: "Seleccioná al menos un cliente para facturar",
      });
      return;
    }

    const mesParaFacturar = previewData.mesFacturacion;

    // ── Guard anti-doble facturación: datos frescos de Firebase ──────────
    const freshSnap = await getDocs(collection(db, "clients"));
    const yaFacturados = [];
    freshSnap.docs.forEach((docSnap) => {
      const cf = { id: docSnap.id, ...docSnap.data() };
      const seleccionado = clientesSeleccionados.some((c) => c.id === cf.id);
      if (seleccionado && clienteYaTieneMesCubierto(cf, mesParaFacturar)) {
        yaFacturados.push(cf);
      }
    });
    ...
    if (yaFacturados.length > 0) {
      Swal.fire({
        icon: "warning",
        title: "Posible doble facturación",
        html: `
          <p><strong>${yaFacturados.length}</strong> cliente(s) ya tienen ese mes cubierto:</p>
          <p style="color:#f57c00">${yaFacturados.map((c) => `${c.name} ${c.lastName}`).join(", ")}</p>
          <p>Cerrá y regenerá el preview.</p>
        `,
      });
      return;
    }
    ...
    const result = await Swal.fire({
      title: "¿Confirmar facturación?",
      html: `
        <p>Clientes: <strong>${clientesSeleccionados.length}</strong></p>
        <p>Total cuotas: <strong>$${getTotalSeleccionados().toLocaleString("es-AR")}</strong></p>
        <p>Mes: <strong>${formatearMesLegible(mesParaFacturar)}</strong></p>
        ${
          mesAFacturar === "actual"
            ? '<p style="color:#f57c00">⚠️ Mes actual</p>'
            : '<p style="color:#1976d2">📅 Mes siguiente (adelantado)</p>'
        }
      `,
      icon: "question",
      showCancelButton: true,
    ...
    try {
      ...
      for (const client of clientesSeleccionados) {
        try {
          ...
          // La deuda actual pasa a "anterior" — siempre, independientemente del mes
          const nuevaDeudaAnterior = deudaAnterior + deudaActual;
          ...
          if (saldoFavor >= cuotaNueva) {
            // El saldo a favor absorbe la nueva cuota
            nuevoSaldoFavor = saldoFavor - cuotaNueva;
            nuevaDeuda = 0;
            nuevoEstado = nuevaDeudaAnterior > 0 ? "Deudor" : "Al día";
          } else {
            // El saldo cubre solo una parte (o nada)
            nuevaDeuda = cuotaNueva - saldoFavor;
            nuevoSaldoFavor = 0;
            nuevoEstado = "Deudor";
          }

          // ✅ Conservar ultimoMesFacturado si ya es posterior
          const ultimoMesExistente = client.ultimoMesFacturado || "";
          const nuevoUltimoMes =
            ultimoMesExistente > mesParaFacturar
              ? ultimoMesExistente
          ...
          console.error(`Error procesando cliente ${client.id}:`, err);
          errores++;
        }
      }
      ...
      Swal.fire({
        ...
        title: "Facturación completada",
        html: `
          <p>✅ Facturados: <strong>${procesados}</strong></p>
          ${errores > 0 ? `<p>⚠️ Errores: <strong>${errores}</strong></p>` : ""}
          <p>Total: <strong>$${getTotalSeleccionados().toLocaleString("es-AR")}</strong></p>
          <p>Mes: <strong>${formatearMesLegible(mesParaFacturar)}</strong></p>
        `,
        timer: 4000,
      });
      ...
      Swal.fire({ icon: "error", title: "Error al procesar la facturación" });
    } finally {
      setIsProcessing(false);
    }
  };
            ...
            <>
              ...
              <Box sx={{ mb: 2 }}>
                <Button
                  ...
                  startIcon={<SettingsIcon />}
                  fullWidth
                  onClick={() => setShowPriceUpdate((v) => !v)}
                  sx={{ mb: showPriceUpdate ? 2 : 0 }}
                >
                  {showPriceUpdate
                    ? "Ocultar"
                ...
                <Collapse in={showPriceUpdate}>
                  <Paper sx={{ p: 2, bgcolor: "#f5f5f5" }}>
                    ...
                    <Alert severity="info" sx={{ mb: 2, fontSize: "0.85rem" }}>
                      Los cambios se guardarán permanentemente en Firebase.
                    </Alert>
                    <Grid container spacing={2}>
                      {updatedActivities.map((activity) => (
                        <Grid item xs={12} sm={6} key={activity.id}>
                    ...
                    >
                      <Button
                        ...
                        onClick={() => {
                          ...
                          setEditingActivityPrices(reset);
                          setUpdatedActivities([...activities]);
                        }}
                      >
                        Restablecer
              ...
              <Alert severity="info" sx={{ mb: 2 }}>
                ...
                {mesAFacturar === "siguiente" && (
                  <Typography
                    ...
                    sx={{ mt: 0.5 }}
                  >
                    ℹ️ La deuda actual del cliente pasará a "deuda anterior" al
                    facturar.
                  </Typography>
                )}
              </Alert>
              ...
              {previewData.clientesAFacturar.length > 0 && (
                <Box sx={{ mb: 3 }}>
                  ...
                  >
                    ...
                    <Button size="small" onClick={toggleSelectAll}>
                      {previewData.clientesAFacturar.every(
                        (c) => selectedClients[c.id],
                      )
                        ? "Deseleccionar todos"
                        : "Seleccionar todos"}
                    ...
                    <Table size="small" stickyHeader>
                      <TableHead>
                        <TableRow>
                          <TableCell padding="checkbox">
                            <Checkbox
                              checked={previewData.clientesAFacturar.every(
                                (c) => selectedClients[c.id],
                              )}
                              onChange={toggleSelectAll}
                              size="small"
