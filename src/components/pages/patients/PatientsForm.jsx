...
const useAutoPayment = () => {
  const registerAutoPayment = async (
    patientData,
    activityPrice,
    collectionName = "patientPayments"
  ) => {
...
export const PatientForm = ({
  ...
  const handleChange = (e) => {
    ...
    // Si es un cliente existente y estamos cambiando actividad o sessions
    const isDebtAffectingChange =
      patientSelected && (name === "actividad" || name === "sessions");
    ...
    const calculatedDebt = calcularDeuda(updatedActividad, updatedSessions);
    ...
    try {
      ...
      if (patientSelected) {
        ...
        const activityPrice = selectedActivity
          ? (selectedActivity.valor || 0) * newPatient.sessions
          : 0;
        ...
        // Registrar pago automático para nuevos pacientes
        if (activityPrice > 0) {
    ...
    >
      ...
      {!patientSelected && (
        <Alert severity="info" sx={{ mb: 2 }}>
          ...
          inicial por la actividad seleccionada.
        </Alert>
      )}
      ...
      {showDebtRecalculation && (
        ...
        >
          <Typography variant="body2">
            <strong>¡Atención!</strong> Los cambios en la actividad o sesiones
            afectarían la deuda.
            <br />
